...
def add_seat_to_row(chart, row_index, seat_to_add)
  # take a chart (2d array) and add seat_to_add to the end of the row that is at row_index index of the chart, then return the chart
  chart[row_index].push(seat_to_add)
  chart
end

def add_another_row(chart, row_to_add)
  # take a chart and add row_to_add to the end of the chart, then return the chart.
  chart << row_to_add
  chart
end
...
def delete_row_from_chart(chart, row_index)
  # take a chart and delete the row at row_index of the chart, then return the chart
...
def count_empty_seats(chart)
  ...
  result = 0
  chart.each_with_index do |row, row_idx|
...
def find_favorite(array_of_hash_objects)
  ...
  answer = array_of_hash_objects.each do |hash|
    hash.each do |key, value| 
      return Hash[hash.to_a.reverse] if key == :is_my_favorite? && value == true
      end 
    end
    return nil if answer.respond_to?(:to_h)
